...
setnames(X.gbm,colnames(X.gbm),new = X.id)

# Gene-id extract

i.na <- which(apply(X.gbm,1,function(x) {sum(!is.na(x))==0}))
X.entity <- X.gbm[-i.na,]
dim(X.entity)
...
cor.prom <- corMX1(selected.prom[1:2500],w=0,X.entity,M.entity,gid,prom=prom.2kb,subset='ALL')
...
X.gbm[gid[1:5],.SD,.SDcols=keys]
X.gbm[gid[1:5],.SD,.SDcols=-key(X.gbm)]
